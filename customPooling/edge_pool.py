...
class EdgePooling(torch.nn.Module):
    ...
    def __init__(self, in_channels, edge_score_activation=None, 
                edge_score_method=None, dropout=0, add_to_edge_score=0.5):
        super(EdgePooling, self).__init__()
        self.in_channels = in_channels

        if edge_score_activation is None:
            edge_score_activation = self.compute_edge_score_softmax
        self.compute_edge_activation = edge_score_activation

        if edge_score_method is None:
            edge_score_method = self.compute_edge_score_concat_linear
        self.compute_edge_score = edge_score_method

        if self.compute_edge_score == self.compute_edge_score_concat_linear:
            self.vars = [torch.nn.Linear(2 * in_channels, 1)]
        elif self.compute_edge_score == self.compute_edge_score_subtract_linear:
            self.vars = [torch.nn.Linear(in_channels, 1)]
    ...
    @staticmethod
    def compute_edge_score_concat_linear(x, edge_index, vars):
        lin = vars[0]
    ...
    @staticmethod
    def compute_edge_score_subtract_linear(x, edge_index, vars):
        lin = vars[0]
        #TODO: Replace with symmetric operator
        e = x[edge_index[0]] - x[edge_index[1]]
        e = lin(e).view(-1)
        return e
    ...
    def forward(self, x, edge_index, batch):
        ...
        """
        e = self.compute_edge_score(x, edge_index, self.vars)
        e = F.dropout(e, p=self.dropout, training=self.training)
        e = self.compute_edge_activation(e, edge_index, x.size(0))
...
class EdgePoolingConcatLinear(EdgePooling):
    def __init__(self, in_channels):
        super().__init__(in_channels, None,
            self.compute_edge_score_concat_linear)

class EdgePoolingSubtractLinear(EdgePooling):
    def __init__(self, in_channels):
        super().__init__(in_channels, None,
            self.compute_edge_score_subtract_linear)